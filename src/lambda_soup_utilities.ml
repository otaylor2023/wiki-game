...
(* |> $$ "li" |> List.concat_map ~f:(fun li -> List.map ~f:(fun li -> texts
   li |> String.concat ~sep:"" |> String.strip)) *)
...
let get_first_item_of_second_unordered_list contents : string =
  let open Soup in
  let uls = parse contents $$ "ul" |> to_list in
  let ul2 = List.nth_exn uls 1 in
  ul2 $ "li" |> texts |> String.concat ~sep:"" |> String.strip
;;
